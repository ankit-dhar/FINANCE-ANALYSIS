...
select count(*) [Total number of accounts]
from Accounts;

--3. What is the total balance of all checking accounts?
select sum(Balance) [Total Balance]
from Accounts
...
SELECT b.BranchName,ROUND(AVG(Balance),2) [TOTAL AMOUNT]
FROM Accounts AS a
JOIN Branches AS b 
...
SELECT  CONCAT(c.FirstName,' ',c.LastName) [FULL NAME]
		,ROUND(MAX(Balance),1) [TOTAL AMOUNT]
FROM Accounts AS a
JOIN 
	Customers AS c
...
SELECT	CONCAT(c.FirstName,' ',c.LastName) [FULL NAME],
		COUNT(t.TransactionID) [TOTAL NUMBER OF TRANSACTIONS]
...
SELECT  CONCAT(c.FirstName,' ',c.LastName) [FULL NAME]
		,ROUND(SUM(Balance),1) [TOTAL AMOUNT]
FROM Accounts AS a
JOIN 
	Customers AS c
	ON A.CustomerID=C.CustomerID
...
SELECT 
	b.BranchName,
	COUNT(t.TransactionID) [NUMBER OF TRANSACTIONS]
FROM Transactions AS t
JOIN 
	Accounts AS a
